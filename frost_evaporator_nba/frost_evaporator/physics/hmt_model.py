...
class HeatMassTransferModel:
    ...
    def update_properties(self, state: FrostEvaporatorState, inputs: FrostEvaporatorInputs):
        ...
        # Use a simple ratio first; only use Log logic if we are safely in the middle
        ...
        if ratio >= 1.0:
            ...
            # We try to find the exact area split, so that the 2ph zone provides exactly the required 
            # capacity to boil, and the superheated zone provides the rest.
        ...
        # Leaving 0.1% of a zone helps the gradients stay alive for the solver
        ...
        m_dens, m_thick, m_flux = self._calculate_mass_flow_split_Fick(
            state.frost.thickness, state.frost.density, m_dot_frost_total,
            rho_w_surf, rho_w_base, state.frost.A_frost_surface
        )
        ...
        state.hmt.set("m_dot_thickening_flux", m_flux)
    ...
    def _process_two_phase_zone(self, state, inputs, eta_surface, area_fraction) -> Dict:
        ...
        # Resistances
        R_frost = self._calculate_resistance_frost(state.frost.thickness, state.frost.k_frost, state.frost.A_frost_surface)
        ...
        def residual_func(T_surf_guess: float) -> Tuple[float, float, float, float, float]:
        ...
        Q_zone, T_s, T_b, w_sat, m_dot_zone = self._find_equilibrium_temperature(
            state, inputs, residual_func, T_ref_lower=T_ref
        )
    ...
    def _process_superheated_zone(self, state, inputs, eta_surface, area_fraction) -> Dict:
        ...
        def residual_func(T_surf_guess: float) -> Tuple[float, float, float, float, float]:
            ...
            if C_ref <= 1e-9:
                ...
                Q_max = C_ref * (T_surf_guess - T_ref_in)
                Q_sink = eff_ref * Q_max
                T_base = T_surf_guess - (Q_sink * R_frost_zone)
        ...
        # Solve for Equilibrium
        Q_zone, T_s, T_b, w_sat, m_dot_zone = self._find_equilibrium_temperature(
            state, inputs, residual_func, T_ref_lower=T_ref_in
        )

        return {"Q": Q_zone, "m_dot": m_dot_zone, "T_surf": T_s, "T_base": T_b}

    def _find_equilibrium_temperature(self, state, inputs, residual_func: Callable, T_ref_lower: float) -> Tuple[float, float, float, float, float]:
        """
        ...
        # Hard Bounds: T_surface cannot be lower than refrigerant or higher than air inlet
        T_min = T_ref_lower + 1e-4  
        T_max = inputs.air.T_in - 1e-4

        # Initial Guess
        T_curr = state.hmt.T_frost_surface
        R_curr, Q_curr, T_base, w_sat, m_dot_val = residual_func(T_curr)
        
        # Quick exit if we are already converged
        ...
        R_prev, _, _, _, _ = residual_func(T_prev)
        ...
        for _ in range(MAX_ITER):
            ...
            if abs(T_new - T_curr) < 1e-5:
                ...
                R_curr, Q_curr, T_base, w_sat, m_dot_val = residual_func(T_new)
    ...
    def _calculate_heat_transfer_air_to_frost(self, state: FrostEvaporatorState, inputs: FrostEvaporatorInputs, T_surface_guess: float, eta_surface: float, area_fraction: float) -> tuple[float, float, float]:
        ...
        Returns:
            ...
            - epsilon_m: Mass transfer effectiveness [-]
        """
        ...
        # Calculate Mass Flow
        m_dot_frost_final = epsilon_m * m_dot_dry_zone * (inputs.air.W_in - w_sat_surf)
    ...
    # ==============================================================================
    # Surface Efficiency and Geometry Calculations
    # ==============================================================================
    ...
    def _calculate_mass_flow_split_Fick(self, frost_thickness: float, frost_density: float, m_dot_total: float, 
        ...
        if m_dot_total > 0.0:
            ...
            porosity = max(0.001, min(0.999, porosity)) # Clamp to avoid div/0
            ...
            # Clamping
            m_dot_densification = max(0.0, min(m_dot_densification, m_dot_total))
