...
excel_filename = 'betriebspunkte_mit_feuchte.xlsx'
...
air_vol_flows = [0.5, 0.6, 0.7]

# --- GELÖSCHT: humidities = [...] --- (Das kommt jetzt aus der Excel)
...
for index, row in df_betriebspunkte.iterrows():
    ...
    # --- NEU: Feuchte direkt aus der Zeile lesen ---
    ...
    for v_flow in air_vol_flows:
        ...
        # --- GELÖSCHT: for phi in humidities: ---
        # (Die folgenden Zeilen sind jetzt eine Ebene weniger eingerückt)
        ...
        for sh in superheats:
            for n_rel in speeds_rel:
                ...
                # Feuchte aus der Excel-Zeile übergeben
                control_inputs.set(
                    name="phi",
                    value=current_phi,  # <--- Hier wird der Wert aus der Zeile genutzt
                    unit="-",
                    description="Relative Humidity Ambient"
                )
...
output_filename = "SC_Propane_Optimized.xlsx"
