"""
Skript zur Berechnung und Auswertung der Messunsicherheiten der Verdampferbilanz.
Dieses Skript liest stationäre Betriebspunkte aus einer Excel-Datei ein, berechnet
die Verdampferleistung sowie deren erweiterte Messunsicherheit basierend auf dem
GUM-Framework (Guide to the Expression of Uncertainty in Measurement) unter
Verwendung der `gum_ebc`-Bibliothek.

Zusätzlich wird eine Sensitivitätsanalyse (Leave-One-Out-Methode) durchgeführt,
um die dominanten Unsicherheitsquellen (Sensoren vs. Klemmen) zu identifizieren,
und es werden diverse Visualisierungen (Plots, Heatmaps) exportiert.
"""
...
# Import der GUM-spezifischen Klassen zur Unsicherheitsfortpflanzung
...
# =============================================================================
# 0) USER SETTINGS / GLOBALE KONFIGURATION
# =============================================================================

# Pfad zur Excel-Datei mit den stationären Betriebspunkten (Simulationsergebnisse)
...
# Definition der Ausgabeordner für die Resultate, Plots und Debug-Dateien
OUTPUT_DIR = Path(
...
# Erweiterungsfaktor (Coverage Factor) k für die erweiterte Unsicherheit
# k=2 entspricht einem Konfidenzintervall von ca. 95 % (Standard nach GUM)
K_EXPANSION = 2
...
# -------------------------------------------------------------------------
# SENSITIVITÄTSANNAHME VOR DEM EEV (Thermodynamischer Workaround)
# -------------------------------------------------------------------------
# Ohne internen Wärmeübertrager (IHX) ist in der Simulation eine Unterkühlung
# von 0 K vorgegeben. Dadurch liegt der thermodynamische Zustand vor dem
# Expansionsventil (EEV) exakt auf der Siedelinie (Sättigungszustand, x=0).
# Die Stoffwertberechnung h(T,p) über CoolProp/Pyfluids ist dort oft instabil.
#
# Workaround:
# Für diese Sensitivitätsvariante wird die Temperatur vor dem EEV für die
# Enthalpieberechnung künstlich um 0.5 K (bzw. hier 1 K) abgesenkt.
# Dadurch wird das Kältemittel sicher in das Einphasengebiet (unterkühlte
# Flüssigkeit) verschoben, wodurch h(T,p) fehlerfrei berechnet werden kann.
#
# Vorteil:
# T_before_EEV bleibt als Eingangsgröße in der Unsicherheitsanalyse erhalten
# und kann weiterhin analytisch perturbiert werden.
#
# Wenn der Sättigungsfehler weiterhin auftritt, testweise auf 1.0 K erhöhen.
# -------------------------------------------------------------------------
BEFORE_EEV_SUBCOOLING_SHIFT_K = 1

# Definition der Hardware-Messkette für die Temperatur
TEMP_SENSOR_TYPE = "PT100A"
TEMP_TERMINAL_CLASS = EL3202_0010

# Definition der Hardware-Messkette für den Druck (allgemein)
PRESSURE_TERMINAL_CLASS = EL3154

# Definition der Hardware-Messkette für den Massenstrom (Coriolis)
MASSFLOW_SENSOR_TYPE = "6400c_DN08"
MASSFLOW_TERMINAL_CLASS = EL3182

# Messbereich des Massenstromsensors in kg/s
...
# Vergleichsvarianten am Verdampfer / Niederdruckseite (Sensor-Auswahl)
# Hier wird untersucht, wie sich die Wahl eines 10-bar vs. 25-bar Drucksensors
# auf die Gesamtunsicherheit der Verdampferleistung auswirkt.
EVAP_PRESSURE_RANGE_VARIANTS = [
    ...
    },
    {
        ...
        "pressure_full_scale": 10 * 1e5,  # Pa
    },
]

# Druckmessung vor dem EEV (Flüssigkeitsleitung / Hochdruckseite)
BEFORE_EEV_PRESSURE_SENSOR_TYPE = "ICS_Schneider_IMP331-p40"
BEFORE_EEV_PRESSURE_FULL_SCALE = 40 * 1e5  # Pa

# =============================================================================
# 1) LABELS FÜR PLOTS UND TABELLEN
# =============================================================================
# Diese Dictionaries dienen der nutzerfreundlichen Formatierung von Achsen-
# und Legendenbeschriftungen in den generierten Diagrammen.
...
# Kürzel für die Betriebspunkte, um Plot-Achsen übersichtlich zu halten
BP_LABELS = {
...
# Standard-Reihenfolge der Zustandsgrößen in Balkendiagrammen
STATE_ORDER = [
    "m_ref",
    "T_out_evap",
    "p_out_evap",
...
def short_bp_name(bp):
    """Gibt einen verkürzten, plot-freundlichen Namen für den Betriebspunkt zurück."""
    return BP_LABELS.get(bp, bp.replace("BP", "").replace("_", " "))


def short_state_name(state):
    """Gibt das definierte Kürzel für eine Zustandsgröße zurück."""
...
def zero_uncertainty(obj):
    """
    Entfernt die in Sensor/Klemme hinterlegten Unsicherheitsanteile temporär.

    Wird für die Sensitivitätsanalyse (Leave-One-Out-Methode) benötigt.
    Hierbei wird iterativ immer eine Unsicherheitsquelle (z.B. Temperatur-Klemme)
    "ausgeschaltet" (u = 0), während alle anderen aktiv bleiben. Die Differenz
    der Gesamtvarianzen zeigt dann den Beitrag der jeweiligen Quelle.
    """
    if hasattr(obj, "a"):
        obj.a = []
    return obj
...
def flags_with_disabled_component(disabled_component=None):
    """
    Erstellt ein Dictionary mit booleschen Flags für alle Hardware-Komponenten.
    Ist `disabled_component` definiert, wird exakt dieser eine Eintrag auf False gesetzt.
    """
    flags = {
        "m_sensor": True,
        "m_terminal": True,
...
# =============================================================================
# 3) MESSOBJEKTE INITIALISIEREN (GUM_EBC INSTANZEN)
# =============================================================================
# Die folgenden Funktionen erzeugen Measurement-Objekte. Diese Objekte enthalten
# neben dem eigentlichen Messwert (x) auch die Information über dessen Unsicherheit (u),
# berechnet aus den Spezifikationen von Sensor und Auswerteklemme.

def create_temperature(T_K, active_sensor=True, active_terminal=True):
    """Erstellt ein Measurement-Objekt für eine Temperaturmessstelle."""
    values = [float(T_K)]
...
def create_pressure(
    ...
    """Erstellt ein Measurement-Objekt für eine Druckmessstelle."""
    values = [float(p_Pa)]
...
def create_massflow(
    ...
    """
    Erstellt ein Measurement-Objekt für eine Massenstrommessstelle (Coriolis).
    Coriolis-Sensoren haben oft Temperatur- und Druckabhängigkeiten, daher
    werden p und T mit übergeben.
    """
    values = [float(m_kg_s)]
...
def enthalpy_refrigerant(p, T):
    """
    Berechnet die Enthalpie über die in `gum_ebc` integrierte
    Pyfluids/CoolProp-Wrapper-Klasse. Die Unsicherheiten von p und T pflanzen
    sich hierbei in die Enthalpie fort (partielle Ableitungen nach GUM).

    Voraussetzung:
    PyfluidsEnthalpy.py verwendet intern FluidsList.nPropane (für R290).
    """
...
# =============================================================================
# 4) EXCEL EINLESEN UND DATEN VORBEREITEN
# =============================================================================

def normalize_column_name(col):
    """Entfernt Umbrüche und Leerzeichen an den Rändern für stabilen Spaltenabgleich."""
    return str(col).strip().replace("\n", " ")


def find_column(df, startswith_text):
    """Findet eine notwendige Spalte in der Excel-Tabelle anhand des Namensanfangs."""
...
def find_optional_column(df, startswith_text):
    """Sucht nach einer optionalen Spalte. Gibt None zurück, falls nicht vorhanden."""
    for col in df.columns:
        col_norm = normalize_column_name(col)
        if col_norm.startswith(startswith_text):
            return col
...
def optional_float(row, col):
    """Konvertiert einen Zellwert sicher zu float, fängt NaNs und Nones ab."""
    if col is None:
        return np.nan
...
def load_operating_points(excel_path):
    """
    Liest die stationären Simulationsdaten der Betriebspunkte aus der Excel-Datei
    und wandelt sie in eine standardisierte Liste von Dictionaries um.
    """
    df = pd.read_excel(excel_path)

    # Identifikation der relevanten Spalten in der Excel
    col_state = find_column(df, "State Number")
    col_desc = find_column(df, "Punktbeschreibung")

    col_m = find_column(df, "m_flow_ref in kg/s")

    col_T1 = find_column(df, "T_1 in K")  # Zustand 1: Verdampfer Austritt
    col_p1 = find_column(df, "p_1 in Pa")

    col_T3 = find_column(df, "T_3 in K")  # Zustand 3: Vor Expansionsventil
    col_p3 = find_column(df, "p_3 in Pa")

    col_T4 = find_optional_column(df, "T_4 in K")  # Zustand 4: Verdampfer Eintritt
    ...
    # Optionale Temperaturdifferenzen zur reinen Auswertung
...
# =============================================================================
# 5) VERDAMPFERBILANZ (KERN DER UNSICHERHEITSFORTPFLANZUNG)
# =============================================================================
...
def calculate_evaporator_balance(
    ...
    """
    Berechnet die Verdampferleistung (Q_evap) mitsamt GUM-Unsicherheitsobjekten.
    Formel: Q_evap = m_ref * (h_out - h_in)
    Dabei gilt h_in = h_before_EEV (Annahme: Isenthalpe Drosselung).

    Wird `disabled_component` übergeben, wird die Unsicherheit dieser Komponente
    auf 0 gesetzt (für die Varianzanalyse in `contribution_analysis`).
    """
    ...
    # Enthalpie inkl. Fehlerfortpflanzung aus T_out und p_out
    ...
    # -------------------------------------------------------------------------
    # Vor EEV: Zustand 3 (Hochdruckseite vor Expansion)
    #
    # Simulation ohne IHX:
    # Unterkühlung = 0 K -> Punkt liegt numerisch auf Sättigungslinie.
    #
    # Für diese Sensitivitätsvariante:
    # T_before_EEV wird für h(T,p) künstlich um den Shift-Wert abgesenkt.
    ...
    # da CoolProp bei exakt x=0 (Sättigung) die Ableitung dh/dT oft nicht bilden kann.
    # -------------------------------------------------------------------------
    ...
    # Enthalpie inkl. Fehlerfortpflanzung aus T_before_EEV und p_before_EEV
    h_before_EEV = enthalpy_refrigerant(
        p=p_before_EEV,
        T=T_before_EEV
    )

    # Annahme: Isenthalpe Drosselung über das Expansionsventil
    ...
    # -------------------------------------------------------------------------
    # Verdampferleistung (Teilen durch 1000 für Wandlung von W in kW)
    # -------------------------------------------------------------------------
    Q_evap = (m_ref * (h_out - h_in_evap)) / 1000

    # Erweiterung der Messunsicherheit (Coverage Factor k=2 -> ~95% Konfidenz)
    if expand:
    ...
    # Rückgabe aller berechneten Objekte als Dictionary
    return {
        "Q_evap": Q_evap,
...
def contribution_analysis(bp, evap_pressure_variant):
    """
    Ermittelt den prozentualen Anteil jeder Messkette (Sensor und Klemme)
    an der Gesamtunsicherheit der Verdampferleistung über die
    Leave-One-Out-Methode (V_i = V_gesamt - V_ohne_i).
    """

    # 1. Berechnung der kompletten Unsicherheit (Alle Quellen aktiv)
    ...
    # Liste aller Hardwaresschnittstellen zur sequenziellen Abschaltung
    components = [
    ...
    # 2. Iteratives Ausschalten jeder Quelle
    for state_variable, source, disabled_component in components:
        ...
        # Varianzanteil der isolierten Komponente i
        variance_i = variance_full - variance_without_i

        # Numerische Stabilität (falls durch Rundungsfehler minimal negativ)
        if variance_i < 0:
            variance_i = 0.0
    ...
    # Gesamtsumme der Einzelvarianzen bilden zur prozentualen Auswertung
    ...
    # Hauptverursacher der Unsicherheit bestimmen (höchster prozentualer Anteil)
    dominant = max(
        contribution_rows,
        key=lambda r: r["share_percent"]
    )

    # Zusammenfassung aller Resultate für diesen Betriebspunkt (Zeile für den Export)
    result_row = {
        "state": bp["state"],
        "description": bp["description"],
        "bp": bp["name"],
        ...
        "Q_evap_kW": Q_full.x,
        "u_Q_evap_kW": Q_full.u,  # Standardunsicherheit (k=1)
        "U_Q_evap_kW": Q_expanded.u,  # Erweiterte Unsicherheit (k=2)
        ...
        # Ergebnisse der Dominanzanalyse
        ...
        # Zum Vergleich: Enthalpien direkt aus der Simulation
...
# =============================================================================
# 7) DEBUG-FUNKTIONEN UND PLAUSIBILITÄTSCHECKS
# =============================================================================
# Diese Sektion vergleicht die im Skript berechneten Stoffwerte mit den
# Resultaten der Modelica/Dymola-Simulation (aus Excel). Dies ist wichtig, um
# Diskrepanzen (z. B. durch unterschiedliche Referenzzustände in CoolProp vs.
# Modelica) aufzudecken.

def export_debug_plausibility_table(results_df, output_dir):
    """Erstellt eine CSV, in der die Skript-Enthalpien den Sim-Enthalpien gegenübergestellt werden."""
    ...
    # Abweichung in Prozent (Vorsicht bei Enthalpien, da diese referenzabhängig sind)
...
def print_debug_summary(result_row):
    """Gibt eine kurze Konsolenausgabe des Vergleichs Sim vs. Skript pro BP aus."""
    Q_from_sim_h = (
...
def plot_debug_q_comparison(debug_df, output_dir):
    """Plottet ein Balkendiagramm der Verdampferleistungen (Simulation vs. Skript)."""
    df = debug_df.copy()
...
def plot_debug_enthalpy_difference(debug_df, output_dir):
    """Visualisiert die relative Abweichung der berechneten Enthalpien."""
    df = debug_df.copy()
    df = df[df["pressure_variant"] == "p25"].copy()
...
# =============================================================================
# 8) PLOTS UND FINALISIERUNG
# =============================================================================
# Funktionen zum Export fertiger Grafiken für die Ausarbeitung.

def plot_one_matrix_per_bp(contrib_df, results_df, output_dir):
    """
    Erstellt pro Betriebspunkt ein Balkendiagramm (Subplots für 25bar/10bar),
    das den Varianzanteil jeder Messkette aufteilt in "Sensor" und "Klemme".
    """
    for bp in contrib_df["bp"].drop_duplicates():
        ...
        for ax, variant in zip(axes, ["p25", "p10"]):
            ...
            # Gruppieren nach Sensor und Klemme für Stacked Bar Chart
            ...
            # Einblenden der absoluten und relativen Unsicherheiten als Textbox
            r = results_df[
                (results_df["bp"] == bp)
...
def plot_sensor_range_comparison(results_df, output_dir):
    """
    Vergleicht die gesamte erweiterte Unsicherheit U(Q_evap) der Verdampferbilanz
    für einen 25-bar- vs. einen 10-bar-Drucksensor und visualisiert die
    prozentuale Reduzierung der Unsicherheit.
    """
    ...
    # Daten des Vergleichs parallel noch als CSV speichern
...
def plot_clean_heatmap(results_df, output_dir):
    """
    Zeichnet eine aggregierte Heatmap über alle Betriebspunkte hinweg, um
    die relative Messunsicherheit (rel. U) visuell auf einen Blick zu erfassen.
    """
    df = results_df.copy()
    ...
    # Pivot für 2D-Matrix (Zeilen = BP, Spalten = Sensortyp)
    heat = df.pivot_table(
        index="bp_short",
    ...
    # Werte direkt in die Matrix schreiben
...
def export_dominant_contributions(contrib_df, output_dir):
    """
    Extrahiert die Top 3 Unsicherheitsquellen pro Betriebspunkt und Variante
    und speichert diese als übersichtliche CSV. Optimal für die Auswertung/Schlussfolgerung.
    """
    rows = []
...
def main():
    ...
    # Hinweis bezüglich des thermodynamischen Workarounds auf der Konsole ausgeben
    print(
    ...
    # Hauptschleife: Berechnung für jeden BP und jede Sensor-Variante
    for bp in operating_points:
        for variant in EVAP_PRESSURE_RANGE_VARIANTS:
            ...
            # Durchführung der Varianz- und Unsicherheitsanalyse
    ...
    # ----- Post-Processing: Debugging und Visualisierungen -----
