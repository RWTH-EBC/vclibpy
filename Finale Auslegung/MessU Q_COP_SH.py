...
# angenommen, damit die Enthalpieberechnung numerisch stabil bleibt.
...
# gesetzt, damit im EnergyMeter-Modell der 1%-Pfad gewählt wird.
...
def enthalpy_refrigerant(p, T):
    """
    Enthalpieberechnung über PyfluidsEnthalpy.
    Voraussetzung:
    PyfluidsEnthalpy.py nutzt intern FluidsList.nPropane.
    """
...
def saturation_temperature_nominal_K(p_Pa):
    ...
    try:
        ...
        # Fallback, falls deine pyfluids-Version anders aufgebaut ist
        # -> ggf. manuell anpassen, falls notwendig
        raise RuntimeError(
...
def saturation_temperature_from_pressure(p_measurement):
    ...
    Die Standardunsicherheit wird per numerischer Sensitivität bestimmt.
    """
...
def calculate_condenser_heat_flow(bp, disabled_component=None):
    ...
    # Massenstrom
    # für den Sensor selbst spielen p/T im aktuellen Modell praktisch keine Rolle
    m_ref = create_massflow(
        bp["m_ref_kg_s"],
        p_for_sensor=p2,
        T_for_sensor=T2,
...
# =============================================================================
# 6) METADATA ZUR BEITRAGSANALYSE
# =============================================================================
