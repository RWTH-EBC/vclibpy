...
# --- KONFIGURATION ---
FLUID = "Propane"  # Kältemittel R290

# Anlagen- und Komponentengrenzwerte für die Validierung
LIMITS = {
    'MAX_T_DISCHARGE': 110.0,  # Maximale Heißgastemperatur in °C
    'MAX_P_CON_BAR': 27.0,  # Maximaler Kondensationsdruck in bar
    'MAX_P_EL_KW': 5.0,  # Maximale elektrische Leistungsaufnahme in kW
    'MAX_RPM': 6600  # Maximale Verdichterdrehzahl
}
...
# Definition der Verdichter-Hüllkurve (Envelope) basierend auf Herstellerdaten
envelope_points_sat = [
    (25, 70), (25, 25), (10, 10), (-35, 10),
    (-35, 65), (-10, 80), (-5, 80), (15, 80), (25, 70)
]
# Transformation der Hüllkurve auf die realen Anlagenbedingungen (inkl. Pinch/Offset)
...
def load_and_map_data(filepath):
    """
    Lädt die Rohdaten aus vclibpy, normalisiert die Spaltennamen und
    prüft die Betriebspunkte gegen die anlagenspezifischen Limits (Envelope, Druck, Temp).
    """
    ...
    # Mapping-Dictionary zur Übersetzung der vclibpy-Ausgaben in gut lesbare Variablen
    col_map = {
        ...
        # Thermodynamische Zustände (1-7) übersetzen
    ...
    # Umrechnung in für die Auswertung praktikable Einheiten (kW, rpm, °C, bar, kJ/kg)
    if 'P_el_W' in df.columns: df['P_el_kW'] = df['P_el_W'] / 1000
    if 'Q_con_W' in df.columns: df['Q_con_kW'] = df['Q_con_W'] / 1000
    if 'n_Hz' in df.columns: df['n_rpm'] = df['n_Hz'] * 110  # Polpaarzahl/Übersetzung spezifisch
    ...
    # Envelope Checks & technische Limit-Validierung
    limit_t = float(LIMITS['MAX_T_DISCHARGE'])
    limit_p = float(LIMITS['MAX_P_CON_BAR'])

    df['T_dis_C'] = df['T2'] if 'T2' in df.columns else (df['T_2_K'] - 273.15 if 'T_2_K' in df.columns else 0)
    df['p_con_check'] = df.get('p3', df.get('p2', 0))

    def get_fail_reason(row):
        """Klassifiziert den Grund für das Scheitern eines Betriebspunkts."""
        reasons = []
    ...
    def check_env(row):
        """Prüft, ob der Betriebspunkt innerhalb der Verdichter-Hüllkurve liegt."""
...
# --- TEIL 2: BOXPLOTS & VIOLINPLOTS ---
def create_boxplots_internal(df, output_folder):
    """Generiert statistische Verteilungen (Drücke und Temperaturen) zur Auswertung."""
    ...
    def sort_key(name):
        if name.startswith('dp'): return 100
        try:
            return int(''.join(filter(str.isdigit, name)))
        except:
            return 50

    # A) DRUCK Verteilungen
    p_cols = sorted([c for c in df_valid.columns if (c.startswith('p') and len(c) <= 3 and c[1:].isdigit())],
                    key=sort_key)
    if p_cols:
...
def export_sensor_data(df, output_folder):
    """
    Erstellt eine aggregierte Tabelle der minimalen und maximalen
    Zustandsgrößen. Dient als direkte Datengrundlage für die Sensorauslegung.
    """
    df_valid = df[df['Limit_OK']].copy()
    ...
    summary_data = []
    m_min = df_valid['m_flow_ref'].min() if 'm_flow_ref' in df_valid.columns else None
    m_max = df_valid['m_flow_ref'].max() if 'm_flow_ref' in df_valid.columns else None
    ...
    for i in range(1, 8):
        ...
        row['m_flow_min [kg/s]'], row['m_flow_max [kg/s]'] = m_min, m_max
...
# --- TEIL 4: LOG-P-H DIAGRAMME ---
def create_logph_diagrams(df, output_folder):
    """
    Zeichnet für jeden simulierten Betriebspunkt das thermodynamische
    Kreisprozessdiagramm (log-p-h) unter Verwendung der CoolProp Bibliothek.
    """
    ...
    try:
        # Generierung der Nassdampfglocke für R290
        ...
        # Isothermen im überhitzten Bereich
    ...
    for idx, row in df.iterrows():
        try:
            ...
            p_vals.append(p_vals[0])  # Zyklus schließen
        except:
            continue
...
def create_scatter_plots(df, output_folder):
    """
    Erstellt Operationskarten (Envelope-Plots). Wenn mehrere Betriebspunkte pro
    Temperaturkombination existieren, wird der "Best-Of" (höchster COP / größte Leistung) visualisiert.
    """
    ...
    for cfg in plots:
        ...
        if m_ok_env.any():
            sc = ax.scatter(current_df.loc[m_ok_env, 'Source_C'], current_df.loc[m_ok_env, 'Flow_C'],
                            c=current_df.loc[m_ok_env, col], cmap=cfg['cmap'], s=120, marker='o', edgecolors='k',
                            label='Gültig (Best-Of)', zorder=5)
        ...
        if mask_fail.any():
            ...
            if 'Fail_Reason' in fails.columns:
                for reason, grp in fails.groupby('Fail_Reason'):
                    ...
                    ax.scatter(grp['Source_C'], grp['Flow_C'], c=style['color'], marker=style['marker'], s=100, lw=2,
