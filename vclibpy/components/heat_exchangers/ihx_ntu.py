...
class IHX_NTU(InternalHeatExchanger):
    ...
    def calc(self, inputs: Inputs, fs_state: FlowsheetState) -> (float, float):
        ...
        # vorsichtshalber cp highside runtersetzen
        state_help_cp = self.med_prop.calc_state(
            "PT",
            state_high_second_regime.p,
            state_high_second_regime.T - 5.0
        )

        m_flow_secondary_cp = self.m_flow_high * (
                (state_high_second_regime.h - state_help_cp.h) /
                (state_high_second_regime.T - state_help_cp.T)
        )
        # Ende cp
