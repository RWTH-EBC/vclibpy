...
class SimpleLMTD(BasicLMTD, abc.ABC):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(SimpleLMTD, self).__init__(flow_type,
...
class SimpleLMTDCondenser(SimpleLMTD):
    """
    ...
    def calc(self, inputs: Inputs, fs_state: FlowsheetState) -> (float, float):
        ...
        Q_lmtd = self.calc_Q_lmtd(lmtd=lmtd,
                                  alpha_pri=alpha_ref_wall,
                                  alpha_sec=alpha_med_wall,
                                  A=self.A)

        error = (Q_lmtd / Q - 1) * 100
        # Get possible dT_min:
...
class SimpleLMTDEvaporator(SimpleLMTD):
    """
