...
EXPERIMENT_DATASETS = [
    ("Wang2014",
     "/Users/wooseok/Desktop/Abbildungen/plotdigitizer/CSV_aus_Paper/Wang_2014_R290_with_ID_6mm_L_1550mm/T_sat-35_G73_5_q33_6.csv",
     "o"),
]
...
G = 73.5  # Mass flux in kg/m2s
Q_FLUX = 33.6  # Heat flux in kW/m2
...
COMPARE_WITH_GW1986 = True  # Compare with Gungor-Winterton (1986)
COMPARE_WITH_GW1987 = True # Compare with Gungor-Winterton (1987)
...
if COMPARE_WITH_GW1986:
    from vclibpy.components.heat_exchangers.heat_transfer.gw1986_htc import (
        GungorWintertonTwoPhase86
    )

if COMPARE_WITH_GW1987:
    from vclibpy.components.heat_exchangers.heat_transfer.gw1987_htc import (
        GungorWintertonTwoPhase87
    )
...
if __name__ == "__main__":
    ...
    if COMPARE_WITH_GW1986:
        gw86 = GungorWintertonTwoPhase86(orientation="horizontal")
        print("[Gungor-Winterton (1986)] Model initialized for comparison")

    if COMPARE_WITH_GW1987:
        gw87 = GungorWintertonTwoPhase87(orientation="horizontal")
        print("[Gungor-Winterton (1987)] Model initialized for comparison")
    ...
    if COMPARE_WITH_GW1986:
        ...
        for x in x_grid:
            fs.set("seg_x", float(x))
            try:
                htc = gw86.calc(st_liq, st_vap, st_liq, st_vap, med, inputs, fs, m_flow)
                alpha_gw.append(float(htc))
            except Exception as e:
                print(f"[Warning] G&W 1986 calculation failed at x={x:.2f}: {e}")
                alpha_gw.append(np.nan)

    # Calculate HTC using Gungor-Winterton 1987 model if enabled
if COMPARE_WITH_GW1987:
    alpha_gw1987 = []
    for x in x_grid:
        fs.set("seg_x", float(x))
        try:
            htc = gw87.calc(st_liq, st_vap, st_liq, st_vap, med, inputs, fs, m_flow)
            alpha_gw1987.append(float(htc))
        except Exception as e:
            print(f"[Warning] G&W 1987 calculation failed at x={{x:.2f}}: {{e}}")
            alpha_gw1987.append(np.nan)
...
if COMPARE_WITH_GW1987:
    ...
    for label, path, marker in EXPERIMENT_DATASETS:
        ...
        try:
            ...
            # Calculate error metrics for Gungor-Winterton 1987 model if enabled
            if COMPARE_WITH_GW1987:
                h_gw87_interp = np.interp(
                    x_exp, x_grid, alpha_gw1987, left=np.nan, right=np.nan
                )
                valid_mask_gw87 = np.isfinite(h_gw87_interp)
                if np.any(valid_mask_gw87):
                    diff_gw87 = h_gw87_interp[valid_mask_gw87] - h_exp[valid_mask_gw87]
                    mae_gw87 = float(np.mean(np.abs(diff_gw87)))
                    mape_gw87 = float(
                        np.mean(np.abs(diff_gw87) / np.maximum(1e-9, h_exp[valid_mask_gw87]))
                    ) * 100.0
                    print(f"[{label}] G&W (1987): N={valid_mask_gw87.sum()} MAE={mae_gw87:.0f} W/m2K MAPE={mape_gw87:.1f}%")

            if COMPARE_WITH_GW1986:
                h_gw_interp = np.interp(
                    x_exp, x_grid, alpha_gw, left=np.nan, right=np.nan
                )
                valid_mask_gw86 = np.isfinite(h_gw_interp)

                if np.any(valid_mask_gw86):
                    diff_gw86 = h_gw_interp[valid_mask_gw86] - h_exp[valid_mask_gw86]
                    mae_gw86 = float(np.mean(np.abs(diff_gw86)))
                    mape_gw86 = float(
                        np.mean(np.abs(diff_gw86) / np.maximum(1e-9, h_exp[valid_mask_gw86]))
                    ) * 100.0
                    print(f"[{label}] G&W (1986): N={valid_mask_gw86.sum()} MAE={mae_gw86:.0f} W/m2K MAPE={mape_gw86:.1f}%")
    ...
    model_tags = ["LW"]
    if COMPARE_WITH_GW1986:
        model_tags.append("GW1986")
if COMPARE_WITH_GW1987:
    model_tags.append("GW1987")
    if COMPARE_WITH_KANDLIKAR:
