...
class GungorWintertonTwoPhase(TwoPhaseHeatTransfer):
    ...
    *per-segment* quantities to be provided via FlowsheetState before `calc`
    ...
    def calc(
        ...
        # Reduced pressure and nucleate boiling contribution (Cooper-type)
        _, p_c, _ = med_prop.get_critical_point()
        p_r = p / p_c
        if not (1.0e-3 < p_r < 0.99):
            raise ValueError(f"reduced pressure p_r={p_r:.3f} outside valid range")

        # Molar mass from MedProp
        M = float(med_prop.get_molar_mass()) * 1000.0  # kg/mol -> kg/kmol
        if M <= 0.0:
            raise ValueError("invalid molar mass: must be > 0")
